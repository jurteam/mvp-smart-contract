...
contract('Arbitration - Amendment with no dispute', function (accounts) {
  ...
  it("12. party2 agrees amendment", async () => {
    //No additional funding required from party 2
    await arbitration.agreeAmendment({from: party2});
    let state = await arbitration.state();
    assert.equal(state, 1);
    assert.equal((await arbitration.dispersal(party1)).toNumber(), 0);
    assert.equal((await arbitration.dispersal(party2)).toNumber(), 180);
    assert.equal((await arbitration.funding(party1)).toNumber(), 100);
    assert.equal((await arbitration.funding(party2)).toNumber(), 80);
    //Excess funding should have refunded
    assert.equal((await token.balanceOf(party1)).toNumber(), 0);
    assert.equal((await token.balanceOf(party2)).toNumber(), 20);
  });

  it("13. party1 proposes amendment", async () => {
    //Original values: [0, 180], [100, 80], "Do some other work..."
    //No additional funding required from party 1
    await token.mint(party1, 80, {from: accounts[0]});
    await token.approve(arbitration.address, 80, {from: party1});
    await arbitration.proposeAmendment([0, 180], [180, 0], "Do some other work...", {from: party1});
    let state = await arbitration.state();
    assert.equal(state, 1);
    assert.equal((await token.balanceOf(party1)).toNumber(), 0);
  });

  it("14. party1 unagrees amendment", async () => {
    //No additional funding required from party 1
    await arbitration.unagreeAmendment({from: party1});
    let state = await arbitration.state();
    assert.equal(state, 1);
    assert.equal((await token.balanceOf(party1)).toNumber(), 80);
  });

  it("15. party1 agrees arbitration", async () => {
  ...
  it("17. party1 withdraws dispersal (zero tokens)", async () => {
  ...
  it("18. party2 withdraws dispersal (180 tokens) - state is now Closed", async () => {
    ...
    //Party 1 dispersal is 0
    assert.equal(finalBalance.sub(initialBalance).toNumber(), 180);
    let state = await arbitration.state();
