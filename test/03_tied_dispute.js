...
contract('Arbitration - Tied dispute', function (accounts) {
  ...
  it("9. start voting resulting in a tie", async () => {
    ...
    //Party1 has 40 votes, Party2 has 40 votes
    assert.equal((await arbitration.totalVotes()).toNumber(), 80);
  ...
  it("11. additional votes placed (under 5%)", async () => {
    await arbitration.vote(party1, 4, {from: voter2});
    //Party1 has 44 votes, Party2 has 40 votes
    assert.equal((await arbitration.totalVotes()).toNumber(), 84);
    assert.equal((await arbitration.partyVotes(party1)).toNumber(), 44);
    assert.equal((await arbitration.partyVotes(party2)).toNumber(), 40);
  ...
  it("13. voters receive rewards", async () => {
    ...
    //Voter1 gets loses 20 staked tokens
    //Voter2 gets 30 reward tokens, and their original 34 tokens
    //Voter3 gets loses 20 staked tokens
    assert.equal(voter1FinalBalance.sub(voter1Balance).toNumber(), 0);
    assert.equal(voter2FinalBalance.sub(voter2Balance).toNumber(), 64);
    assert.equal(voter3FinalBalance.sub(voter3Balance).toNumber(), 0);
  });
  ...
  it("14. parties receive payouts", async () => {
    ...
    assert.equal(party1FinalBalance.sub(party1Balance).toNumber(), 70);
